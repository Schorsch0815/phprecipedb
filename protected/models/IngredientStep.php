...
class IngredientStep extends CFormModel
{
    ...
    public function validateIngredients()
    {
        ...
        for ($i = 0; $i < sizeof($ingredients); ++$i) {
            ...
            $lPossibleUnits = $connection->createCommand()
                ->select('id,short_desc')->from('tbl_unit')
                ->where(
                    'short_desc=:name or description=:name',
                    array(':name' => $ingredients[$i][2]))->queryAll();
            ...
            $lPossibleIngredients = $connection->createCommand()
                ->select('id,name,unit_usage')->from('tbl_ingredient')
            ...
            if (sizeof($lPossibleIngredients) > 0) {
                ...
                $lPossibleIngredients = $connection->createCommand()
                    ->selectDistinct('id,name,unit_usage')
                    ->from('tbl_ingredient')
                    ->where(
    ...
    public function unparseIngredients()
    {
        ...
        foreach ($this->parsedIngredients as $ingredientData) {
            ...
            $xxx = array_key_exists('0', $ingredientData[1]);
            FB::log("Result is $xxx");

            $this->ingredients .= ('' == $ingredientData[0]) ? ''
                : ($ingredientData[0] . ' ');
