...
class ApLinkPager extends CBasePager
{
        ...
        for ($i = 0; $i < $labelCount; ++$i)
            $buttons[] = $this
                ->createPageButton(
                    ...
                    !in_array($labels_comp[$i], $activeLabels_comp),
                    $i + 1 == $currentPage);
        ...
        $selected)
    {
        if ($hidden || $selected)
            $class .= ' '
                . ($hidden ? self::CSS_HIDDEN_PAGE : self::CSS_SELECTED_PAGE);
        return '<li class="' . $class . '">'
            . CHtml::link($label, $this->createPageUrl($page)) . '</li>';
    }
