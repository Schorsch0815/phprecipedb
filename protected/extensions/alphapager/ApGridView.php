...
class ApGridView extends CGridView
{
    ...
    public function init()
    {
        ...
        if ($this->enableAlphaPagination
            && $this->dataProvider->getAlphaPagination() === false)
        ...
        if ($this->ajaxUpdate !== false) {
            ...
            if ($class == 'ApLinkPager') {
                $cs
                    ->registerScript(
                        __CLASS__ . '#' . $id,
                        "jQuery('#" . $id . " ." . $this->alphaPagerCssClass
                            . " a').live('click',function(){ $.fn.yiiGridView.update('"
                            . $id
                            . "',{url: $(this).attr('href')});return false; });");
            } else if ($class == 'ApListPager') {
                $cs
                    ->registerScript(
                        __CLASS__ . '#' . $id,
                        "jQuery('#" . $id . " ." . $this->alphaPagerCssClass
                            . " select').live('change',function(){ $.fn.yiiGridView.update('"
                            . $id . "',{url: $(this).val()});return false; });");
