    ...
    public function run()
    {
        ...
        if ($this->showAllPage) {
            ...
            if (!count($activeLabels) > 0)
                $disabledOptions[$this->createPageUrl(-1)] = array(
                        'disabled' => true);
        }
        ...
        if ($this->showNumPage) {
            ...
            if (!$this->pages->activeNumbers)
                $disabledOptions[$this->createPageUrl(0)] = array(
                        'disabled' => true);
        }

        for ($i = 0; $i < $labelCount; ++$i) {
            $pages[$this->createPageUrl($i + 1)] = $this
                ->generatePageText($labels[$i]);
            if (!in_array($labels_comp[$i], $activeLabels_comp))
                $disabledOptions[$this->createPageUrl($i + 1)] = array(
                        'disabled' => true);
        }

        if (!empty($disabledOptions))
            $this->htmlOptions['options'] = array_merge(
                (array) $this->htmlOptions['options'],
                $disabledOptions);
