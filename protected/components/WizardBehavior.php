...
class WizardBehavior extends CBehavior
{
     ...
     */
    public $events = array('onFinished' => 'wizardFinished',
            'onProcessStep' => 'wizardProcessStep', 'onStart' => 'wizardStart',
     ...
     * @see getMenu()
     */
    public $menuProperties = array('id' => 'wzd-menu',
            'activeCssClass' => 'wzd-active', 'firstItemCssClass' => 'wzd-first',
    ...
    public function attach($owner)
    {
        if (!$owner instanceof CController)
            throw new CException(
                Yii::t('wizard', 'Owner must be an instance of CController'));
    ...
    public function process($step)
    {
        ...
        if (empty($step)) {
            ...
            if ($this->isValidStep($step)) {
                ...
                if (!$this->forwardOnly
                    && isset($_REQUEST[$this->previousButton]))
                    $this->previousStep();
    ...
    {
        if (sizeof($data) !== 3 || !is_array($data[0]) || !is_array($data[1])
            || !(is_integer($data[2]) || is_null($data[2])))
            return false;
    ...
    {
        $this
            ->_session[$this->_stepsKey][(empty($step) ? $this->_currentStep
                : $step)] = $data;
    }
    ...
    {
        return (empty($step) ? $this->_session[$this->_stepsKey]->toArray()
    ...
    public function getMenu()
    {
        if (null === $this->_menu) {
            ...
            $this->_menu = $this->getOwner()
                ->createWidget('zii.widgets.CMenu', $properties);
        }
    ...
    private function generateMenuItems()
    {
        ...
        foreach ($this->_steps as $step) {
            ...
            if ($previous
                && !empty($this->menuProperties['previousItemCssClass']))
                $item['itemOptions'] = array(
                        'class' => $this
                            ->menuProperties['previousItemCssClass']);

            $items[] = $item;
        }
        if (!empty($this->menuLastItem))
            $items[] = array('label' => $this->menuLastItem, 'active' => false);
    ...
    protected function nextStep()
    {
        if ($this->autoAdvance)
            ...
            $step = ($index < $this->_steps->count() ? $this->_steps[$index]
                : null);
        }
    ...
    {
        return (isset($this->_session[$this->_branchKey]) ? $this
                ->_session[$this->_branchKey] : new CMap());
    }
    ...
    protected function isValidStep($step)
    {
        ...
        if ($index >= 0) {
            if ($this->forwardOnly)
                return $index
                    === $this->_steps->indexOf($this->getExpectedStep());
        ...
        foreach ($steps as $label => $step) {
            ...
            if (is_array($step)) {
                foreach (array_keys($step) as $branchName) {
                    ...
                    if (($branchDirective
                        && $branchDirective === self::BRANCH_SELECT)
    ...
    {
        return (isset($this->_session[$this->_branchKey]) ? $this
                ->_session[$this->_branchKey][$branch] : null);
    }
    ...
    {
        $event = new WizardEvent($this, $this->_currentStep,
            $this->read($this->_currentStep));
    ...
    {
        $event = new WizardEvent($this, $step,
            array($this->read(), $this->branches()->toArray(),
